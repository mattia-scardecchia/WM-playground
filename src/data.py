...
class NoisyVectorWorld:
    ...
    """
    def __init__(self, D: int = 102, signal_dim: int = 2, noise_dim: int = 100, step: float = 1.0, seed: int = 0):
        assert signal_dim == 2, "This toy env assumes a 2-D signal part."
        assert D == signal_dim + noise_dim, "D must equal signal_dim + noise_dim"
        self.D = D
...
def sample_trajectories(env: NoisyVectorWorld, n_traj: int, traj_len: int, policy: str = "random") -> Dict[str, np.ndarray]:
    N = n_traj; T = traj_len
    s_list, a_list, sp_list = [], [], []
    s = env.init_state(N)
    for _ in range(T):
        if policy == "random":
            a = env.rng.randint(0, 4, size=(N,))
        else:
            raise ValueError(f"Unknown policy: {policy}")
        sp = env.step(s, a)
        s_list.append(s.copy()); a_list.append(a.copy()); sp_list.append(sp.copy())
        s = sp
    s_arr = np.concatenate(s_list, axis=0)
    a_arr = np.concatenate(a_list, axis=0)
    sp_arr = np.concatenate(sp_list, axis=0)
    return {"s": s_arr.astype(np.float32),
